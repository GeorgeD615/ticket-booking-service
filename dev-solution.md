...
## Введение
Система бронирования билетов — это распределённый highload-сервис для продажи билетов на мероприятия (концерты, матчи, спектакли), спроектированный с учётом высокой конкурентной нагрузки и строгой консистентности данных. Основная задача системы — корректно обрабатывать одновременный доступ тысяч пользователей к ограниченному ресурсу (местам в зале), предотвращая двойную продажу и обеспечивая стабильную работу в моменты пикового спроса. Архитектура построена на микросервисном подходе с использованием PostgreSQL как источника истины и Redis для кэширования, распределённых блокировок и временных резервирований, а также включает механизмы управления нагрузкой (Virtual Waiting Queue) для обеспечения предсказуемого поведения системы при экстремальных всплесках трафика.
...
| Термин | Определение |
|--------|------------|
| Событие (Event) | Концерт, спектакль, матч или другое мероприятие, на которое продаются билеты. |
| Площадка (Venue) | Физическое место проведения события с определённой схемой зала и набором мест. |
| Место (Seat) | Конкретная позиция в зале, доступная для бронирования в рамках события. |
...
| Позиция брони (Booking item) | Элемент брони, связанный с конкретным местом события (booking item). |
| Заказ (Order) | Подтверждённая покупка после успешной оплаты, фиксирующая факт продажи билетов. |
| Элемент заказа (Order item) | Детализация заказа по конкретным местам, включая snapshot данных на момент покупки. |
| Таймаут брони (Booking timeout) | Ограниченное время удержания мест за пользователем до автоматического освобождения. |
| Oversell | Ситуация, при которой одно и то же место продаётся более одного раза. |
| Идемпотентность (Idempotency) | Возможность безопасно повторить запрос без создания дублей. |
...
При целевой нагрузке в десятки тысяч read-запросов на получение доступности мест и до 1 000 операций бронирования в секунду система требует горизонтального масштабирования PostgreSQL. Основная нагрузка приходится на операции изменения состояния мест (`EVENT_SEAT`), создания броней (`BOOKING`) и заказов (`ORDER`), поэтому данные распределяются между несколькими физическими шардами.

#### Стратегия шардирования

В качестве ключа шардирования используется `event_id`. Все данные, связанные с конкретным мероприятием, размещаются на одном физическом шарде. Такой подход позволяет локализовать все транзакции бронирования внутри одного шарда.

#### Причины выбора `event_id` как shard key

Основная конкурентная нагрузка в билетных системах концентрируется вокруг отдельных популярных мероприятий. Во время старта продаж тысячи пользователей одновременно работают с одним и тем же набором мест (`EVENT_SEAT`). Размещение всех данных события на одном шарде позволяет:

- выполнять ACID-транзакции без распределённых транзакций;
- минимизировать latency операций бронирования;
- избежать сложных джоинов между шардами.

#### Роутинг запросов

`API Gateway` или `Booking Service` определяет физический шард на основе `event_id`. Для этого используется round-robin распределение.
